...
export const bakeTurn = (board: IQuantikCell[][], move: IQuantikPiece) => {
  const boardCopy: IQuantikCell[][] = JSON.parse(JSON.stringify(board));

  const res: IQuantikTurn = {
    board: boardCopy,
    move,
  };
...
export const isPieceValidInRegion = (
  ...
  sameRegionCells.forEach((cell) => {
    i++;
    //console.log(`${i} Reg:${cell.region} ${cell.piece.side} ${cell.piece.shape}, piece:${piece.side} ${piece.shape}`);
...
export const playerHasAvailableMove = (
  ...
  if (!moveAvailable) console.log("PLAYER CANT MOVE");
